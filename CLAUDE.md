...
- `Entities/` — Domain entities prefixed with `D` (e.g., `DStudent`, `DPayment`, `DEnrollment`).
  - Every entity exposes two factory methods:
    - `Create(...)` — for new instances; runs full validation, throws `DomainException` on failure.
    - `Reconstitute(...)` — for rehydrating from the database; skips re-validation.
...
- `Interfaces/` — Split into two families:
  ...
  - `IXxxQueries` — read operations; return DTOs; support `GetPagedAsync(int skip, int take, XxxFilter? filter)` returning `(List<XxxDTO> Items, int Total)`.
  - Other service interfaces: `IUnitOfWork`, `IJwtService`, `IPaymentPreviewCache`.
- `DTOs/` — Named `[Action][Entity]DTO` or `[Entity][Purpose]DTO` (e.g., `CreateStudentDTO`, `StudentListDTO`, `PaymentPreviewResponseDTO`).
